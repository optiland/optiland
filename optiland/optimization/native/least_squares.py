...
class LevenbergMarquardt(SteppedOptimizer):
    ...
    def step(self, state: OptimizationState) -> OptimizationState:
        """Execute one accepted LM step (inner λ-search is hidden inside ``step()``).
        ...
        for op in problem.operands:
            ...
            if eff is not None and not math.isclose(float(eff), float(w), rel_tol=1e-9):
                warnings.warn(
                    ...
                    "sum_squared().  Results may differ from earlier runs of "
                    "LeastSquares on multi-field / multi-wavelength systems.",
                    UserWarning,
                    stacklevel=4,
                )
