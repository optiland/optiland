...
# Below this projection norm the primary basis vector is considered parallel
# to the seed direction and the secondary one is used instead.
_DEGENERATE_PROJECTION = 1e-8
...
class SolveReport:
    ...
    Attributes:
        ...
        fallback_used: Whether a fallback seed/strategy was substituted for
            the requested one anywhere in the pipeline.
    """
...
class BoundLaunch:
    ...
    def __init__(self, param: LaunchParameterization, x, y, z, L, M, N) -> None:
        ...
        if not param.is_infinite:
            ...
            norm = be.where(norm < _DEGENERATE_PROJECTION, 1.0, norm)
            ...
            use_alt = n1 < _DEGENERATE_PROJECTION
            n1_safe = be.where(use_alt, be.ones_like(n1), n1)
            n_alt_safe = be.where(n_alt < _DEGENERATE_PROJECTION, 1.0, n_alt)
            self.e1 = tuple(
    ...
    def project(self, x, y, z, L, M, N) -> tuple:
        ...
        along = be.where(be.abs(along) < _DEGENERATE_PROJECTION, 1.0, along)
